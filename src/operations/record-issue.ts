...
export function runRecordIssue(
  sessionId: string,
  issueNumber: number,
  ...
  const gateResult = checkOperationGate('record-issue', state.state)

  if (!gateResult.pass) {
    return {
      output: formatOperationGateError('record-issue', gateResult.reason),
      exitCode: EXIT_BLOCK,
    }
  }
  ...
  const updatedState: WorkflowState = {
    ...
    eventLog: [
      ...state.eventLog,
      createEventEntry('record-issue', deps.now(), { issue: issueNumber }),
    ],
  }
  deps.writeState(statePath, updatedState)
  return {
    output: formatOperationSuccess('record-issue', updatedState),
    exitCode: EXIT_ALLOW,
  }
}
