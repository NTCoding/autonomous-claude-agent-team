...
export function runSignalDone(
  sessionId: string,
  deps: SignalDoneDeps,
): OperationResult {
  const statePath = deps.getStateFilePath(sessionId)
  const state = deps.readState(statePath)
  const gateResult = checkOperationGate('signal-done', state.state)

  if (!gateResult.pass) {
    return {
      output: formatOperationGateError('signal-done', gateResult.reason),
      exitCode: EXIT_BLOCK,
    }
  }
  ...
  const updatedState: WorkflowState = {
    ...state,
    developerDone: true,
    eventLog: [...state.eventLog, createEventEntry('signal-done', deps.now())],
  }
  deps.writeState(statePath, updatedState)
  return {
    output: formatOperationSuccess('signal-done', updatedState),
    exitCode: EXIT_ALLOW,
  }
}
